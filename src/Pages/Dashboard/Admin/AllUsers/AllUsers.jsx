...
function AllUsers() {
  ...
  const handleAdminRole = (user) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch(`https://12-server-virid.vercel.app/user/admin/${user._id}`, {
          method: "PATCH",
        })
          .then((res) => res.json())
          .then((data) => {
  ...
  const handleInstructorRole = (user) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch(`https://12-server-virid.vercel.app/user/instructor/${user._id}`, {
          method: "PATCH",
        })
          .then((res) => res.json())
          .then((data) => {
  ...
  const handleDelete = (user) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch(`https://12-server-virid.vercel.app/user/delete/${user._id}`, {
          method: "DELETE",
        })
          .then((res) => res.json())
          .then((data) => {
                ...
                <td>
                  <div className="tooltip" data-tip="Delete this user">
                    <button
                      ...
                      className="btn text-lg hover:bg-red-700 bg-white hover:text-white"
                    >
                      <LuTrash />
                    </button>
                  </div>
